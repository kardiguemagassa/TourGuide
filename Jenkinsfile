...
pipeline {
    ...
    // Configuration des outils adaptée pour les deux environnements
    tools {
        maven 'M3'                  // Installation Maven locale
        jdk 'JDK-21'                // Installation JDK locale
    }

    environment {
        // Détermination dynamique des outils en fonction de l'environnement
        IS_DOCKERIZED = sh(script: 'grep -q docker /proc/1/cgroup 2>/dev/null', returnStatus: true) == 0
        MAVEN_HOME = IS_DOCKERIZED ? '/usr/share/maven' : tool('M3')
        JAVA_HOME = IS_DOCKERIZED ? '/usr/lib/jvm/java-17-openjdk-amd64' : tool('JDK-21')
            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        echo "=== ENVIRONNEMENT ==="
                        echo "Mode: ${IS_DOCKERIZED ? 'Docker' : 'Local'}"
